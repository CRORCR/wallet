...
let sendList = JSON.parse(fs.readFileSync('../context/vali/deposit.json'));

if(fs.existsSync('../context/vali/deposit.json')){
    let deposit_send = JSON.parse(fs.readFileSync('../context/vali/deposit.json'));
...
web3.eth.getTransactionCount(sendList.from,function (err,result) {
    if(!err)
    {
        ...
        OTALoop.EndFunc = function () {
            fs.writeFileSync('../context/vali/withNonce.json',JSON.stringify(sendList,null,4),"utf8");
            process.exit();
        }
        OTALoop.run();
    }
